...
var TOKEN_FILE2 = path7.join(CONFIG_DIR, "token.json");
...
var PULL_PLANS_BIN = path10.resolve(HOOK_DIR, "../cli/pull-plans.js");
...
async function main() {
  ...
  const note = auth.signedIn ? [
    "Memlin is active for this workspace. MCP tools and hooks share the same login",
    "(~/.config/memlin/token.json) and refresh automatically. Call memlin_resolve_task",
    "before non-trivial work to load this project's skills, memory, approved goals, schemas, and decisions.",
    auth.expiresInMin != null && auth.expiresInMin <= 120 ? `Token refreshes automatically (expires in ~${auth.expiresInMin} min).` : null
  ].filter(Boolean).join(" ") : null;
  process.stdout.write(
    JSON.stringify({
      env: { MEMLIN_HOST: "cursor" },
      additional_context: [hazardWarning, authNotice, note, handoffContext].filter(Boolean).join("\n\n")
    })
  );
}
main().catch(() => {
