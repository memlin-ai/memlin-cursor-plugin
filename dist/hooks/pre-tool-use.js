...
// apps/cursor-plugin/src/hooks/pre-tool-use.ts
...
function emit(permission, agentMessage, userMessage) {
  const payload = {
    permission,
    agentMessage,
    userMessage
...
async function main() {
  ...
  try {
    ...
    log(
      `cursor pre-tool-use: handler threw (fail-open): ${err instanceof Error ? err.message : String(err)}`
    );
    return emitAllow();
  }
  if (verdict.decision === "allow") return emitAllow();
  const cursorPermission = "deny";
  ...
  const reasonText = `${verdict.reason}${idsSuffix}`;
  const agentMessage = verdict.decision === "block" ? `Blocked by Memlin guardrail: ${reasonText}` : `Needs review per Memlin guardrail: ${reasonText}`;
  const userMessage = verdict.decision === "block" ? `Memlin blocked this tool call. ${reasonText}. Edit the rule at memlin.ai or rerun with the rule disabled.` : `Memlin asked you to review this tool call. ${reasonText}.`;
  return emit(cursorPermission, agentMessage, userMessage);
}
void main();
